...
WeaponAccessoryChords weapon_accessory_chords(
    const ControllerInputState& state,
    const bool gameplay_mode,
    const bool vehicle_mode) noexcept {
    if (!gameplay_mode || !state.radial_menu) return {};
    return {
        state.reload,
        state.fire_mode && !vehicle_mode,
        state.grenade && !vehicle_mode,
        state.swap_weapon && !vehicle_mode,
        state.interact && !vehicle_mode,
    };
}
...
ControllerInputOutput::ControllerInputOutput() {
    ...
    if (turn_rate_size > 0 && turn_rate_size < std::size(turn_rate_value)) {
        ...
        if (parsed >= 50.0F && parsed <= 3000.0F) smooth_turn_counts_per_second_ = parsed;
    }
...
void ControllerInputOutput::update(
    ...
    if (!enabled_ || !game_is_foreground(game_pid)) {
        ...
        light_or_laser_previous_ = state.radial_menu && state.reload;
        deploy_weapon_previous_ = state.radial_menu && state.fire_mode;
        inventory_previous_ = state.radial_menu && state.swap_weapon;
        map_previous_ = state.radial_menu && state.interact;
        reload_chord_consumed_ = state.radial_menu && state.reload;
        fire_mode_chord_consumed_ = state.radial_menu && state.fire_mode;
        grenade_chord_consumed_ = state.radial_menu && state.grenade;
        return;
    }
    ...
    const bool gameplay_mode = (context & game_context_gameplay) != 0U;
    const bool ui_mode = (context & game_context_ui) != 0U && !gameplay_mode;
    const bool mouse_ui_mode = ui_mode &&
        (context & game_context_mouse_ui) != 0U;
    const bool vehicle_mode = (context & game_context_vehicle) != 0U;
    const bool proxy_mode = gameplay_mode &&
        (context & game_option_proxy_active) != 0U;
    if (mouse_ui_mode) {
        // Zeus, configuration screens and foreground DLC dialogs are owned by
        // the physical mouse. Release every synthetic controller input so a
        // trigger/stick cannot click, scroll or steer those interfaces.
        release_all();
        reload_previous_ = state.reload;
        fire_mode_previous_ = state.fire_mode;
        swap_previous_ = state.swap_weapon;
        ...
        grenade_previous_ = state.grenade;
        radial_previous_ = state.radial_menu;
        ui_accept_previous_ = state.fire_mode;
        ui_back_previous_ = state.swap_weapon;
        ui_middle_previous_ = state.sprint_click;
        return;
    }
    const WeaponAccessoryChords accessory_chords = weapon_accessory_chords(
        state, gameplay_mode, vehicle_mode);
    // Once the native map/inventory display is open, gameplay context is
    // intentionally false. Keep only these two modifier chords alive in UI
    // context so the same VR input can close the display again.
    const bool ui_map_chord = ui_mode && state.radial_menu && state.interact;
    const bool ui_inventory_chord =
        ui_mode && state.radial_menu && state.swap_weapon;
    // Left grip is a modifier for low-frequency weapon actions. These are
    // Arma's native defaults: L toggles the mounted laser/flashlight and C
    // toggles weapon deployment/bipod. Grip+B is consumed here while SQF
    // changes proxy/native mode. Process chords before the proxy branch so
    // both native and controller-absolute weapon modes receive them.
    if (proxy_mode) {
        // The proxy SQF owns accessory state as well as the visible laser,
        // because Arma's native L action cannot illuminate a hidden rig.
        light_or_laser_previous_ = accessory_chords.light_or_laser;
    } else {
        tap_on_rising_edge(accessory_chords.light_or_laser,
                           light_or_laser_previous_, 'L');
    }
    tap_on_rising_edge(accessory_chords.deploy_weapon,
                       deploy_weapon_previous_, 'C');
    // Proxy mode opens these displays from SQF. That is independent of the
    // foreground keyboard layout and lets the addon release its scripted
    // camera before Arma creates the map/inventory display. Native mode keeps
    // the same controller chords through Arma's default M/I bindings.
    if (proxy_mode) {
        inventory_previous_ = accessory_chords.inventory;
        map_previous_ = accessory_chords.map;
    } else {
        tap_on_rising_edge(accessory_chords.inventory || ui_inventory_chord,
                           inventory_previous_, 'I');
        tap_on_rising_edge(accessory_chords.map || ui_map_chord,
                           map_previous_, 'M');
    }
    if (accessory_chords.light_or_laser) reload_chord_consumed_ = true;
    if (accessory_chords.deploy_weapon) fire_mode_chord_consumed_ = true;
    if (accessory_chords.toggle_proxy) grenade_chord_consumed_ = true;
    if (!state.reload) reload_chord_consumed_ = false;
    if (!state.fire_mode) fire_mode_chord_consumed_ = false;
    if (!state.grenade) grenade_chord_consumed_ = false;
    if (proxy_mode) {
        // The proxy camera may keep OpenXR focused while the Arma window is no
        // longer the Windows foreground target. SQF therefore owns proxy
        // locomotion and firing directly from shared controller telemetry.
        // Release synthetic keys/buttons here to avoid lost or doubled input.
        set_key('W', false, forward_);
        set_key('S', false, backward_);
        set_key('A', false, left_);
        set_key('D', false, right_);
        set_key(VK_LSHIFT, false, sprint_);
        // Native ADS remains optional: when enabled it is the real Arma
        // right-mouse action, never a synthetic proxy-camera zoom/crosshair.
        set_mouse_button(MOUSEEVENTF_LEFTDOWN, MOUSEEVENTF_LEFTUP,
                         false, fire_);
        set_mouse_button(MOUSEEVENTF_RIGHTDOWN, MOUSEEVENTF_RIGHTUP,
                         state.aim &&
                             (context & game_option_native_ads) != 0U,
                         aim_);
        smooth_turn_residual_ = 0.0F;
        vehicle_pitch_residual_ = 0.0F;

        // Interaction and vault remain ordinary Arma key actions; they do not
        // affect weapon orientation and retain compatibility with scripted
        // doors and mission actions.
        set_key(VK_SPACE, state.interact && !state.radial_menu, interact_);
    ...
    // The real player remains authoritative in every mode. These native mouse
    // actions therefore own the actual muzzle, optic, reload and ammunition.
    set_mouse_button(MOUSEEVENTF_LEFTDOWN, MOUSEEVENTF_LEFTUP,
                     state.fire, fire_);
    set_mouse_button(MOUSEEVENTF_RIGHTDOWN, MOUSEEVENTF_RIGHTUP,
                     state.aim, aim_);

    if (ui_mode) smooth_turn_residual_ = 0.0F;
    if (smooth_turn_enabled_ && !ui_mode && delta_seconds > 0.0F) {
        float turn_rate = smooth_turn_counts_per_second_;
        if ((context & game_option_turn_fast) != 0U) turn_rate = 2400.0F;
        else if ((context & game_option_turn_slow) != 0U) turn_rate = 1050.0F;
        ...
        if (magnitude > stick_threshold_) {
            ...
            smooth_turn_residual_ += normalized * turn_rate * delta_seconds;
    ...
    tap_on_rising_edge(state.reload && !state.radial_menu &&
                           !reload_chord_consumed_,
                       reload_previous_, 'R');
    tap_on_rising_edge(state.fire_mode && !state.radial_menu &&
                           !fire_mode_chord_consumed_,
                       fire_mode_previous_, 'F');
    ...
    set_key(VK_SPACE, state.interact && !state.radial_menu && !ui_mode,
            interact_);
    ...
    tap_on_rising_edge(state.vault, vault_previous_, 'V');
    tap_on_rising_edge(state.grenade && !state.radial_menu &&
                           !grenade_chord_consumed_,
                       grenade_previous_, 'G');
    ...
    // Left grip is the add-on modifier/settings input. Never synthesize
