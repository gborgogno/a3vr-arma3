...
| Left grip | Reserved; no gameplay binding in this alpha |
| Left thumbstick | Move and strafe |
| Right thumbstick horizontal | Faster smooth camera/body turn |
| Right thumbstick up/down | One stance step: stand / crouch / prone |
...
crouch or prone. Return the stick to center before the next step. Two quick
downward flicks now reach prone without waiting for the crouch animation. A
physical HMD drop of approximately 30 cm after recentering requests crouch;
rising above the hysteresis threshold stands again only when room scale caused
that crouch. In menus, map, inventory and Zeus, point with the right controller. A cyan
...
stick up raises one stance level and down lowers one level. In vehicles the
vertical axis controls camera/turret pitch instead, while the horizontal axis
retains smooth turn.
In Zeus, the left stick moves the camera. The default smooth-turn rate is
`1100` counts/second, vehicle pitch uses `950` counts/second and the trigger uses a
...
the external borderless window at 1920x1080 while rendering internally at
3840x2160, raises terrain and object detail, and uses moderate sharpening to
retain distant clarity without excessive headset shimmer. The original
`Arma3.cfg` and player profile are copied to
...
Right-controller motion aim defaults to `900` counts/radian so Arma's native
weapon and body follow normal controller rotation more closely. The launcher
can override this through `A3VR_CONTROLLER_COUNTS_PER_RADIAN`.
Comfort mono is submitted once as a compositor-owned surface shared by both eyes. The
default profile uses one compositor-owned `17.5 x 9.84375` surface at a distance of `5.0`
...
uses a `9.5`-wide quad while preserving the capture aspect ratio, so
